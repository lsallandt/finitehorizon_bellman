...
class Valuefunction_TT:
    ...
    def calc_grad(self, t, x):
        ...
        # if True:
        if ind_2 >= len(self.t_vec_p):
            grad_1 = self.calc_end_reward_grad(t, x)
            # print('grad1.shape after', grad_1.shape)
            # print('t, ind_2 >= len(self.t_vec_p)', t, ind_2, len(self.t_vec_p))
            return grad_1
        else:
            # print('else: ind_1, ind_2', ind_1, ind_2)
            if ind_2 == len(self.t_vec_p) - 1:
            ...
            # print('t, t_vec_p[ind_1], t_vec_p[ind_2]', t, self.t_vec_p[ind_1], self.t_vec_p[ind_2], ind_1, ind_2, la.norm(grad_1), la.norm(grad_2))
            return grad_1 + (t - self.t_vec_p[ind_1])*(grad_2 - grad_1)/(self.t_vec_p[ind_2] - self.t_vec_p[ind_1])
