...
class Pol_it:
    ...
    def build_rhs_batch_experimental(self, samples):
        ...
        # ret = jl.Parallel(n_jobs=2)(jl.delayed(np.sqrt)(i0**2) for i0 in range(samples.shape[1]))
        ...
        # x_mat, u_mat = jl.Parallel(n_jobs=2)(jl.delayed(self.optimize_fun.calc_optimal_control)(samples[:,i0], u_mat[:,i0,:]) for i0 in range(samples.shape[1]))
        # jl.Parallel(n_jobs=2)(jl.delayed(np.sqrt)(i ** 2) for i in range(10))
